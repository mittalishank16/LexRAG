...
def _hf_embed(texts: list[str]) -> list[list[float]]:
    try:
        data = hf_client.feature_extraction(
            text=texts,
        ...
        print(f"Embedding resolution dropout encountered: {e}")
        return [[0.0] * 768 for _ in texts]


def _hf_rerank(query: str, passages: list[str]) -> list[float]:
    try:
        payload = {"source_sentence": query, "sentences": passages}
        resp = hf_client.post(
        ...
        print(f"Reranking processing exception encountered: {e}")
