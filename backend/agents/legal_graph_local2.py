...
# local embedding generator (nomic-embed-text matches standard 768-dim spaces perfectly)
local_embeddings = OllamaEmbeddings(
    model="nomic-embed-text",
    base_url="http://localhost:11434"
)

# Local LLM instance for processing state actions and re-ranking scoring
local_llm_instance = ChatOllama(
    model="llama3.1:8b",
    format="json",
    base_url="http://localhost:11434",
    #temperature=0  
...
def _hf_embed(texts: list[str]) -> list[list[float]]:
    """Local substitute using Ollama embeddings"""
    return local_embeddings.embed_documents(texts)


def _hf_rerank(query: str, passages: list[str]) -> list[float]:
    """Local substitute for text-ranking using local LLM to evaluate match relevance."""
    scores = []
    for passage in passages:
        prompt = (
            f"Given the following query and document passage, rate how relevant the passage is "
            f"to answering the query with a single floating-point number between 0.0 and 1.0.\n"
            f"Query: {query}\n"
            f"Passage: {passage}\n"
            f"Return ONLY the raw numerical score and absolutely nothing else."
        )
        try:
            response = local_llm_instance.invoke(prompt).content.strip()
...
class HFInferenceEmbeddings(Embeddings):
    """Maintains signature continuity so downstream Chroma code runs unchanged."""
...
def strategist_agent(state):
    ...
    s  = main_llm.invoke(p).content.strip().upper()
    if   'BOTH'     in s: s = 'BOTH'
